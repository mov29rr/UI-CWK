...
void WaterDataset::loadData(const std::string& filename) {
  ...
  for (const auto& row : reader) {
    try {
      ...
      WaterSample sample(row["sample.samplingPoint.label"].get<>(), row["determinand.label"].get<>(), level,
                         row["determinand.unit.label"].get<>(), row["sample.isComplianceSample"].get(),
                         row["sample.sampleDateTime"].get<std::string>()

      );
      std::cout << "Created WaterSample: " << sample.getLocation() << ", " << sample.getPollutant() << ", "
                << sample.getLevel() << ", " << sample.getUnit() << ", " << sample.getComplianceStatus() << std::endl
...
std::vector<PollutantSample> WaterDataset::loadPollutantSamples(const std::string& filename, int rowCount) {
  ...
  for (const auto& row : reader) {
    ...
    PollutantSample sample(row["Pollutant"].get<std::string>(),      // Assuming 'name' column exists
