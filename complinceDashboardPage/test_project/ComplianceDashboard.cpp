...
ComplianceDashboard::ComplianceDashboard(QWidget* parent) : QMainWindow(parent) {
  setupUI();
  populateTable("../temp/Y-2024-M.csv");
}
...
std::string ComplianceDashboard::calculateComplianceStatus(const WaterSample& sample,
  ...
  for (int j = 0; j < pollutantSamples.size(); ++j) {
    if (sample.getPollutant() == pollutantSamples[j].getName()) {
      ...
      if (sampleLevel >= minThreshold && sampleLevel <= maxThreshold) {
        return "good";
      } else if (sampleLevel >= minThreshold - 0.2 * thresholdRange &&
                 sampleLevel <= maxThreshold + 0.2 * thresholdRange) {
        return "medium";
      } else {
        return "bad";
      }
...
void ComplianceDashboard::populateStats(const std::string& bestLocation, const std::string& worstLocation,
  ...
  if (selectedLocation == "All Locations") {
    ...
    importantInfo->append(
        QString("<font color='red'>Lowest-performing location: %1</font>").arg(QString::fromStdString(worstLocation)));
  }

  // Show year stats only if "All Years" is selected
  if (selectedYear == "All Years") {
    importantInfo->append(
        QString("<font color='green'>Highest-performing year: %1</font>").arg(QString::fromStdString(bestYear)));
    importantInfo->append(
        QString("<font color='red'>Lowest-performing year: %1</font>").arg(QString::fromStdString(worstYear)));
  }

  // Show pollutant stats only if "All Pollutants" is selected
  if (selectedPollutant == "All Pollutants") {
    importantInfo->append(
        QString("<font color='green'>Most compliant pollutant: %1</font>").arg(QString::fromStdString(bestPollutant)));
    importantInfo->append(
        QString("<font color='red'>Least compliant pollutant: %1</font>").arg(QString::fromStdString(worstPollutant)));
  }
...
void ComplianceDashboard::populateTable(const std::string& filename) {
  ...
  if (samples.empty()) {
    ...
    QMessageBox::warning(this, "No Data Found",
                         "There is no data available in the file. Please download it from - <a "
                         "href='https://environment.data.gov.uk/water-quality/view/download'>this link</a>");
    return;
  }
...
void ComplianceDashboard::applyFilters() {
  ...
  if (selectedYear == "All Years") {
    ...
    for (int year = 2020; year <= 2024; ++year) {
      std::string yearFile = "../temp/Y-" + std::to_string(year) + "-M.csv";
      try {
    ...
    std::string newDate = "../temp/Y-" + selectedYear.toStdString() + "-M.csv";
    try {
